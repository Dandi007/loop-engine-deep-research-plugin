...
| **B2** | ⛔ 端到端：跑完**证据 channel 真的出现 `research.evidence.v2`**，条数 > 0 | 真跑后读 channel 断言；⛔ 不得用打桩 bus 替代 |
...
| **B7** | ⛔ `DD_RUN_ID` / `DD_RUN_ROOT` 显式覆盖时语义逐字不变 | 显式传值，断言落点与旧行为一致 |
...
### 2.1 ⛔ B1/B2 的执行约束
- ⛔ **真机只允许打 `research:p02-smoke-1dce60`**（已是冒烟专用板）
  与一个**已核实存在**的证据 channel；
  ⛔ **不得写 `research:v1-*`**（V1 的干净板面，由 gate 保留）
- 跑前跑后**记录消息数增量**写进 dev-notes；⛔ 增量须 ≤ `--max-writes`
- ⚠️ 证据 channel 必须**先核实存在**再用：`research:p02-smoke-1dce60` **没有** `.evidence` 兄弟 channel，
  ⛔ **不得靠字符串拼接推导 channel 名**（A9 已因此把默认值留空）
