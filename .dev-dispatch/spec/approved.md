# E0c2b —— 回归基线：终止语义域（前一版真机验证不通过后重开）

> **前一版 `dev_dr_e0c2_20260812_1254` 的下场**：三轮 attempt 才过闸门（blocker 3 → 1 → 0），
> 但**派发方真机验证判据 9 时当场不成立**，gate REJECT ⇒ CANCELLED，代码全部作废。
> 下面 GT-5～GT-8 是那三轮 + 真机跑换来的地面真相，**全部实测、逐字照抄，不得推测、不得由 fixture 反推**。
> ⛔ 尤其 GT-6：它是把前一版毙掉的那一条，单测永远看不见它。
