# E0c6 —— 一个失败的 generate worker 不该毙掉整条回归基线

> **前一版 `dev_dr_e0c5_20260813_0555`**：**Z1 与 Z2 第一次真的成立**（详见 GT-17），
> 但**紧接着的第二跑挂了** ⇒ Z3 不成立 ⇒ gate REJECT。
> 根因是它自己 §1.2 的实现：检测到「run 退出却没产出 result」之后**让整个 tick 非零退出**，
> 而不是记录诊断后继续。⇒ **一个失败的 generate worker 就能毙掉整条基线。**
>
> **前一版做对、本包必须原样保留的**（⛔ 不得回退）：
> tick 不再被 30 秒 `node_timeout` 砍掉、`MAX_CLUES=24` 收窄回归范围、
> 驱动响亮报 `TICK FAILURE`、入口具名失败、逐轮 drain 记录归档、耗时分阶段 `timings`。
...
### GT-17　⭐⭐ 同一交付、同一台机器、背靠背两跑：一次 exit 0，一次 exit 5
...
### 1.1 ⭐ 「run 退出却没产出 result」必须**记录并继续**，不得毙掉 tick
...
2. **⭐⭐ 判别性（GT-17）**：构造「一个 generate/worker run 已 exited 但没产出 result」⇒
