...
## 5　真机冒烟（⛔ 不进 acceptance_commands）

**写脚本，但不要在本次派发中运行它。**

`scripts/smoke-cas.ts`：对测试 channel `research:p02-smoke-1dce60` 依次
① 发一条 `research.clue.v2`（status=open）
② `claimClue` → 断言 success
③ 再次 `claimClue` 同一 entity → 断言 `error === "conflict"`
④ 打印三条消息的 `message_id` 与 `channel_seq`

> ⛔ **agent-bus 是 append-only、无 DELETE 路由，任何写入都不可回退。**
> 本线曾为确认体量上限往生产 bus 写了 4 条共 5.3MB 垃圾，**至今清不掉**。
>
> 因此：**冒烟脚本由 gate 阶段人工执行且只执行一次**，实现方**不得**把它接进
> `acceptance_commands`、不得在开发过程中反复运行、不得循环或批量写入。
> 脚本总写入量**硬上限 3 条消息**。

**DoD 归属**：A1–A8 属本次派发的验收；冒烟属 gate 义务。

## 6　变异自检（必须逐断言归因）

| 变异 | 必须杀死 |
|---|---|
| M1 把数值状态码判定改回 `msg.includes("409")` | **A2** |
| M2 把 `getEntity` 的错误区分改回 catch-all `return null` | **A3** |
| M3 把 `afterSeq` 判断改回 falsy（`if (opts.afterSeq)`） | **A4** |
| M4 把 `casUpdateClue` 的 `supersedes` 改成来自第二次独立读的 head | **A6** |
