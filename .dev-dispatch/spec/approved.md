# E0c9 —— 接着 E0c8 的最后一条改（它死于 attempt 7 的 spawn E2BIG）

> **前一版 `dev_dr_e0c8_20260813_1340`**：七轮 attempt，blocker 从 7 一路收到 1，
> 最后 **development 死于基础设施而非代码**：attempt 7 的 implement 节点
> `[节点执行异常] spawn E2BIG` —— 累计的评审反馈把 prompt 撑到超过 `ARG_MAX`，进程起不来。
> ⇒ 换新 development 重开（反馈清零），**只剩下面这一条要改**。
>
> **⛔ 前一版已经做对、必须原样保留的**（continuous review 最后一轮零 blocker）：
> §1.2 的「run 退出无 result ⇒ 记录并继续」及其 triage/generate **两条**判别性用例
> （generate 侧的假板必须含 explored 线索、真能走到 generate 分支）、
> `check-drain-failures` 能识别被引擎杀掉的 tick（`result="[外部调用失败 status=TIMEOUT]"`、`error="exec"`，
> 不只认 `[bash 非零退出 EXIT:n]`）、`MAX_CLUES` 声明并经 `fleet.yaml.tpl` 真正接线、
> `timings` 覆盖整个 tick（含 generate 段）、兜底真能终止、逐轮 drain 记录归档、生产总线护栏。

## ⭐ 本包唯一要修的那条（前一版 final review 的最后一条 blocker，逐字）

```
workflows/deep-research/tick/workflow.yaml 的注释写：
  「取 node_timeout = 900s … ≥ 实测最大单 tick 耗时（904s ≈ 1.004× headroom）」
但 900 < 904.2 —— 既不是 ≥，更没有余量。
配套测试断言的也是错的量：断 limits.node_timeout >= DEFAULT_AGENT_RESULT_TIMEOUT_MS/1000（=900，一个源码常量），
而判据 2 要求断言「不小于交付中记录的实测最大单 tick 耗时」。
```

**改法**：
1. `node_timeout` 取一个**真的大于实测最大值且有余量**的值（实测最大 **904.2 秒**，派发方 2026-08-13 03:46 真机取证；
   建议 1800，并在注释里写清 `1800 / 904.2 ≈ 2.0×` 这一步算术）。
   同时说明它与 `DEFAULT_AGENT_RESULT_TIMEOUT_MS = 900_000`（15 分钟）的关系——
   ⛔ 引擎闸刀必须**大于**一次合法的等待预算，否则合法等待仍会被腰斩。
2. 判据 2 的测试改成断言 **`node_timeout ≥ 交付中记录的实测最大单 tick 耗时 × 声明的余量倍数`**，
   ⛔ 不得断言与某个源码常量对齐；把 `node_timeout` 调回任一小于该实测值的数 ⇒ 测试必须变红。
...
### GT-24　⭐ dd 的 attempt 累积会撑爆 `ARG_MAX`：第 7 轮 implement 直接 `spawn E2BIG`

前一版 attempt 7 的 implement 节点逐字：

```
[节点执行异常] spawn E2BIG
```

⇒ 反馈投影随 attempt 累积增长，到第七轮时 prompt 超过 `ARG_MAX`，**进程根本起不来**。
这是 development 级的硬上限：**一个 development 不要指望能撑到七轮以上**，
返工轮数多时应当换新 development 重开（本包即是）。
⛔ 与实现无关，记录在此仅为解释前一版为何中断。
