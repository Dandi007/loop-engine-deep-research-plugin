...
| D7 | §3 的变异矩阵与 D1–D6 的证据**落成 `docs/dev-notes/dev_ledr_g1_default_budget_02.md`** | 该文件存在且三列齐全 |

> ⚠️ **依赖由 `setup_commands` 提供**：本 development 配置了 `npm ci`，在 acceptance 之前于**同一个 workspace** 执行。
> 上一轮（`dev_ledr_g1_default_budget_01`）**实现与两轮 review 全部 APPROVE，只死在 acceptance 的 `tsc: not found`（exit 127）**——
> dd 的 workspace 是**纯 git checkout，不含 `node_modules`**，而当时没配 `setup_commands`。
> ⇒ **本包不需要为依赖做任何事**，也**不得**把 `node_modules` / `package-lock.json` 的变更写进交付。
