...
#   GT-22：失控兜底必须真的终止循环并非零退出。当 DRAIN_MAX_ATTEMPTS 被撞到（异常情形），
#   响亮失败（HIT ATTEMPT LIMIT），不是"永远不触发"。
WALL_START=$(date +%s)
DRAIN_ATTEMPT=0
...
while true; do
  ...
  # 次数上限检查（失控兜底：GT-22 要求真的终止循环并非零退出）。
  # 正常情形下 wall_clock 先耗尽（自洽校验保证），只有异常情形才会撞到本分支。
  if [ "$DRAIN_ATTEMPT" -ge "$DRAIN_MAX_ATTEMPTS" ]; then
    echo "[e0-regression] HIT ATTEMPT LIMIT: max_attempts=${DRAIN_MAX_ATTEMPTS} drain_attempts=${DRAIN_ATTEMPT} wall_clock_elapsed=${ELAPSED:-0}s (runaway safeguard triggered — GT-22)" >&2
    _last_stdout="$RECORD_DIR/drain-rounds/drain-${DRAIN_ATTEMPT}.stdout.log"
    if [ -f "$_last_stdout" ]; then
      _last_term="$(printf '%s' "$DRAIN_SUMMARY" | node "$PLUGIN_ROOT/scripts/read-termination.mjs" 2>/dev/null)" || _last_term=""
      if [ -n "$_last_term" ]; then
  ...
  if [ "$DRAIN_REASON" = "max_rounds" ]; then
    ...
    # drained ⇒ 继续判终态，但先检查 drain 内是否有 tick 失败（§1.3 / 判据 4）。
    # check-drain-failures.mjs 读 drain summary 与 journal.jsonl，命中 TICK FAILURE ⇒ exit 3。
    set +e
    CHECK_DRAIN_OUT="$(printf '%s' "$DRAIN_SUMMARY" | node "$PLUGIN_ROOT/scripts/check-drain-failures.mjs" 2>&1)"
    CHECK_DRAIN_EXIT=$?
    set -e
    if [ "$CHECK_DRAIN_EXIT" -ne 0 ]; then
      echo "$CHECK_DRAIN_OUT" >&2
      _drain_fail_echo "$DRAIN_ATTEMPT" "$DRAIN_EXIT" "drained_with_tick_failure"
      LOOP_EXIT=5
      break
    fi
  else
