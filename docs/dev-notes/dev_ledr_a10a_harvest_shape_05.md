...
| 变异 | 被杀断言 | 验证 |
|---|---|---|
| **M1** `result.evidences ?? []` 改回 `result.evidence ?? []` | **B3**（`publish 次数 === evidences.length` 挂） | 亲跑 B3 挂；还原后 line 288 逐字复原 |
| **M2** `no_result` 分支 `casExplored: false` 改回 `true` | **B7**（两条断言全挂） | 亲跑 B7 挂；还原逐字复原 |
| **M3** 空 `evidences` 分支 `casExplored: true` 改回 `false` | **B8**（两条断言全挂） | 亲跑 B8 挂；还原逐字复原 |
| **M4** 守卫去掉 required 齐全检查（函数体置空） | **B6**（`no materials key ⇒ WorkerResultShapeError` 挂） | 亲跑 B6 挂；还原后守卫主体逐字复原 |

> 每条变异后 `git diff --stat` 复核还原；M1/M4 破坏后若被误提交，typecheck 也会红灯
> （`result.evidence` 不在类型上 / 守卫不再抛错），故工区干净可被 `npm run typecheck` 二次确认。
> 最终 `git diff --stat` 仅余 3 本产品文件 + 1 新增 schema + 本 dev-notes，无变异残留。
