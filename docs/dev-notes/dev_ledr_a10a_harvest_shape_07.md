...
- 既有 A8e 用例（H1/H6/H7/H8/H9/H12/H13/H14/H16）与 A9 生产路径用例的 worker.result
  夹具同步改为真实形状。
- 新增 B1–B8 硬验收用例。

### `test/a9-tick-trigger.test.ts`
- F9 生产路径的 worker.result.v1 夹具同步改为真实形状（`evidences` / 裸 `proposed_clues` / `materials`）。
...
| B9 | A9 F9、A8f F1/F5、A8e H6/H7/H14 仍成立（原用例未删，夹具已修正形状）✓ |
| B10 | 未触碰 `.dd-evidence/`；既有 270 条用例一条未删（全量 281 条）✓ |
