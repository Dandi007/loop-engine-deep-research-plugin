...
| M7 | dispatch CAS 成功写 run_id 进卡 | `M7`（捕获 cas 入参，断言非空 run_id + in_flight） |
...
| M15 | typecheck + 全量测试 exit 0 | `npm run typecheck` / `npm test` 均绿（168 条全绿） |
| M16 | 既有用例一行未删 | git diff 无 `it(` 净减少（既有 151 条全保留） |

## 变异自检归因

| 变异 | 被杀断言 |
|---|---|
| X1 `runs` 改回硬编码空 | **M1**（M2 仍过——正是本包存在的理由）；M6 grep |
| X2 去掉分页（只读第一页） | M6（3 读 / after_seq） |
| X3 dispatch 不写 run_id 进卡 | M7 |
| X4 CAS 失败不跳过、继续动作 | M8 |
...
- `npm test` —— 既有 151 条 + 新增 16 条（tick-run）全绿（168 条）
