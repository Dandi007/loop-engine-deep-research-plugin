...
- F0 真机端到端：以真实 `bin/deep-research-loop.sh` 在 `research:p02-smoke-1dce60` 上跑，
  drain 输出 `ticksByLabel.tick >= 1`。本包已让「驱动投首个触发 + bun 驱动」成立，
  为 gate 的真机 F0 提供前置条件；单测环境无真实 bus（spec §5 不得触网），故 F0 在 gate 侧执行。
...
真机 F0 跑前跑后记录消息数增量，写入本 dev-notes；⛔ 增量须 ≤ `--max-writes`（默认 5）。
`EVIDENCE_CHANNEL` 若指向不存在的 channel，收割会响亮失败 —— F0 允许 0 张待收割卡，
