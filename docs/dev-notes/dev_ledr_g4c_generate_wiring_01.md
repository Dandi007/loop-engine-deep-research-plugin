...
> `input_commit`: `a9158c942eea09422e6e8e379810226f6cf0edb9`
...
| `src/tick-run.ts` | 触发边：`runChannelWrite` 在 `decideGenerate(termination)` 为真时调用 `runGenerate`；10 个 deps 全部生产接线；一次性保证（module-level Set + 文件标记跨进程持久）；`--origin` CLI 参数；`--doc-channel` CLI 参数；`MissingExportRootError`；`spawnExport` 生产实现创建父目录 |
...
| `bin/deep-research-loop.sh` | 导出 `RESEARCH_ORIGIN`（缺省由 `RESEARCH_QUESTION` 确定性派生，sha256 前 16 位） |
| `workflows/deep-research/fleet.yaml.tpl` | 新增 `research_origin` input |
| `workflows/deep-research/tick/workflow.yaml` | 新增 `research_origin` seed payload |
| `workflows/deep-research/tick/templates/tick.md` | 变量声明 `research_origin`；tick_args 增量追加 `--origin` |
| `test/g4c-generate-wiring.test.ts` | U1–U7 硬验收（20 个用例）：U1/U2/U4 为行为级测试，驱动 `runChannelWrite` 生产入口，注入 spy deps 断言运行时行为；U4 负例驱动生产 `spawnExport`（非零功率套套逻辑） |
...
| **U2** | 行为级：同一 `--origin` 连续两次 `runChannelWrite`，第二次 `generateTriggered === false`，`spawnRole`/`writeDoc`/`spawnExport` spy 各被调用 4/4/1 次（恰好一次）；跨进程持久由文件标记保证 | 通过 |
...
| **T2** | 去掉一次性保证（每个 tick 都跑生成段） | U2 挂 | U2 断言两次 `runChannelWrite` 后 spy 调用次数恰好为 4/4/1；去掉一次性保证（移除 `hasGeneratedInAnyProcess` 检查与 `markGeneratedInAnyProcess` 调用）⇒ 第二次调用 spy 次数翻倍 ⇒ U2 挂 |
