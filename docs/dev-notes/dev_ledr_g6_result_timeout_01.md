...
> 本文件是验收证据。`input_commit` = `226aa5d404ad12c6de9a2d150638e37fd8d5ec82`。
...
- `test/g6-result-timeout.test.ts`：新增 15 个测试（R1–R6）。
...
- **R1 discriminant**：generate `readBody` 在**第 31 次 poll** 后才出现结果（`agentRunsReads >= 31`），仍正确返回。这时若 readBody 被 revert 为固定 30 次轮询，它会在第 30 次后放弃，永远找不到结果 ⇒ 测试挂。证明确在使用时间预算（不是 30×1s 固定次数）。✓
...
所有 G6 测试使用 `AGENT_RESULT_POLL_MS=5` 或 `10`ms，`AGENT_RESULT_TIMEOUT_MS=10` 或 `500`ms。R5 通过 `resolveAgentResultTimeout()` 验证环境变量注入后返回的 `pollMs` 与 `timeoutMs` 确实为小值（≤10ms / ≤500ms），而非断言生产缺省。全量测试时长在讨论中。✓
...
未设置 `ANCHOR_CHECK_BIN` / `DOC_CHANNEL` / `RESEARCH_ORIGIN` / `EXPORT_ROOT` / `AGENT_RESULT_*`。27 files ≥ 基线 26，487 tests ≥ 基线 472。✓

### R8：变异矩阵（逐断言归因，亲跑实测）

| 变异 | 改什么 | 被测断言 | 实测 |
|---|---|---|---|
| **S1** | 生成段 `readBody` 保留 30×1s（只改 triage） | R1 discriminant（生成段那条） | 将生成段 `readBody` 的 `while (Date.now() < deadline)` 循环 revert 为固定 `for (i=0;i<30;i++)`，R1 discriminant 的结果在第 31 次 poll 才出现，30 次固定轮询永远找不到 ⇒ 测试挂。**被杀** ✓。还原干净。生成段与 triage 的 readBody/readResult 是独立循环，各自独立调用 `resolveAgentResultTimeout()`，可单独 revert 任一个。 |
