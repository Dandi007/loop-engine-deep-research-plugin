...
usage:
  ...
  ... --run <channel_id> [--max-writes <n>] [--evidence-channel <evidence_channel_id>] [--allowed-root <path>] [--question <研究主问题>] [--prev-coverage <n>] [--prev-zero-growth-rounds <n>]
...
--run 对显式传入的 channel 执行 CAS 认领/回收 + spawn + 收割（接线判别）：先 CAS 成功才按 role spawn，
      spawn 同步失败当场 CAS 回 open；exited(0) 卡先收割（evidence + 新 clue）再 CAS 到 explored；
       ...
       --prev-coverage / --prev-zero-growth-rounds 跨 tick 经 trigger body 传递的上一轮计数（G4b；缺省 0）；
        拒绝写 v1 冻结 channel。
  JSON 输出含 hasPendingWork：板面是否仍有非终态 clue（proposed/open/in_flight），由板面确定性推出（A9）。
  JSON 输出含 termination：本轮终态判定（G4b；用本轮真实板面 + prev 计数调用 decideTermination）。
`;
