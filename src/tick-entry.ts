...
--run 对显式传入的 channel 执行 CAS 认领/回收 + spawn + 收割 + 生成段（接线判别）：先 CAS 成功才按 role spawn，
      spawn 同步失败当场 CAS 回 open；exited(0) 卡先收割（evidence + 新 clue）再 CAS 到 explored；
       ...
       --origin 本次研究 id（生成段产物回写 origin；缺省时不触发生成段）；
  ...
  JSON 输出含 generateTriggered：本 tick 是否触发了生成段（runGenerate 被调用且非空转；G4c）。
