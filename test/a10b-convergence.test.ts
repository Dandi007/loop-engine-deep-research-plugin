  ...
  try {
    out = execFileSync("bash", [SCRIPT], {
      ...
      env: {
        ...
        // The supplied engine prefers its host-level registry when present. That
        // registry uses a newer chain schema, so pin this compatibility test to
        // the registry shipped with the exact engine under test.
