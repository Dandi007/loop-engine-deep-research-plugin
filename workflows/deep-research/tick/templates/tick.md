...
# ⛔ G4a——`--question`（研究主问题）也随装配系统一路注入（bin 导出 RESEARCH_QUESTION，无内置缺省，
#    未配置即响亮失败）；`--run` 带上 `--question`，triage 决策不再因缺 question 响亮失败。
...
if [ -n "$tick_channel" ]; then
  # G4a —— 增量拼 argv（spec §1.2）：不再展开 evidence_channel × allowed_root × question 的
  #  组合分支树（4 → 8 → 16），而是逐参数追加后一次调用；set -euo pipefail 下数组空值安全。
  # ⛔ run_output 的捕获与后续 hasPendingWork 判定逐字不变（那是 A9 的续投逻辑，本包不碰）。
  tick_args=("$tick_entry" --run "$tick_channel")
  if [ -n "$evidence_channel" ]; then
    tick_args+=(--evidence-channel "$evidence_channel")
  fi
  if [ -n "$allowed_root" ]; then
    tick_args+=(--allowed-root "$allowed_root")
  fi
  tick_args+=(--max-writes "$max_writes")
  if [ -n "$research_question" ]; then
    tick_args+=(--question "$research_question")
  fi
