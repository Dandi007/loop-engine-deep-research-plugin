...
if [ -n "$tick_channel" ]; then
  ...
  # E0c4 —— 单个 tick 的声明上界（ms）：由 profile 声明，经 pipeline input 注入，缺省空（不设限）。
  if [ -n "$tick_timeout_ms" ]; then export TICK_TIMEOUT_MS="$tick_timeout_ms"; fi
  # E0c4 —— 回归基线收窄 maxClues：由 profile 声明，经 pipeline input 注入，缺省空（DEFAULT_TICK_CONFIG 兜底）。
